...
def build(scan_canonical_path: Path, run_id: str, out_dir: Path):
    ...
    for it in items:
        ...
        by_path_norm.setdefault(path_norm, asset_id)
        ...
        coverage["file.path_rel"] += 1
        coverage["file.path_norm"] += 1
        if size_bytes is not None:
            coverage["file.size_bytes"] += 1
        if mtime_raw is not None:
            coverage["file.mtime_raw"] += 1
    ...
    (out_dir / "evidence_pack.stats.json").write_text(json.dumps(stats, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    sample = {
        ...
        "nodes": pack["nodes"][:40],
        "edges": pack["edges"][:60],
        "indexes_keys": list(pack["indexes"].keys())
    }
    (out_dir / "evidence_pack.sample.json").write_text(json.dumps(sample, ensure_ascii=False, indent=2), encoding="utf-8")
